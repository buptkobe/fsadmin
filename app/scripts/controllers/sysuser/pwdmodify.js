...
angular.module('sbAdminApp')
  .controller('PwdModifyCtrl', function ($scope, SysuserFactory, $state, toastr,store) {
        var vm = $scope.vm = {
            show_error: false,
            show_type: 3,
            item: {}
        };
        vm.item.name = store.get('username');

        vm.submit = function (basic_form) {
            vm.show_error = true;
            basic_form.$setDirty();
            if(basic_form.$valid){
                SysuserFactory.update(vm.item, function(response) {
                	if (response) {
